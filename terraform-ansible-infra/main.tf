...
resource "aws_instance" "app_server" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      echo "app_server ansible_host=${self.public_ip} ansible_user=ubuntu ansible_ssh_private_key_file=~/.ssh/microservices.pem ansible_ssh_common_args='-o StrictHostKeyChecking=no'" >> ansible/inventory
    EOT
  }
}
